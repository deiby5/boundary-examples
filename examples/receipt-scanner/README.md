...
A CLI receipt scanner that extracts structured receipt data from receipt images using AI. It demonstrates how the [Boundary](https://withboundary.com) contract library validates and auto-repairs LLM outputs.
...
2. The image is sent to OpenRouter with instructions to return structured JSON.
...
- `node/src/scan.ts` wraps the OpenRouter call in `receiptScanContract.accept()`.
...
- `python/src/scan.py` wraps the OpenRouter call in `receipt_scan_contract.accept()`.
